...
class JwtUtil {
    ...
    public static function getAuthorizationHeader() {
        $headers = null;
        // Check all possible header variations, including more REDIRECT_ prefixes
        $possibleHeaders = [
            'Authorization',
            'HTTP_AUTHORIZATION',
            ...
            'REDIRECT_REDIRECT_REDIRECT_REDIRECT_HTTP_AUTHORIZATION',
            'REDIRECT_REDIRECT_REDIRECT_REDIRECT_REDIRECT_HTTP_AUTHORIZATION',
            'HTTP_X_AUTHORIZATION'
        ];
        foreach ($possibleHeaders as $header) {
            if (isset($_SERVER[$header])) {
                $headers = trim($_SERVER[$header]);
                break;
            }
        }
